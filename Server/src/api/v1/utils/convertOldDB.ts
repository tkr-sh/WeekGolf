                ...
                async (err, rep) => {
                    const newData = rep.map(
                        async obj => {
                            ...
                            // If it's 'just' a golf 
                            else if (languagesList.includes(obj.title.split(" ").at(-1).slice(0, -1).toLowerCase())) {
                            ...
                            if (lang !== null) {
                                ...
                                if (obj.major === 2) {
                                    ...
                                    const content = obj.content.toLowerCase().replace(/[^\d\w\s]/g, '').split(" ")

                                    let firstByteIndex: number | null = content.indexOf("bytes");
                                    let secondByteIndex: number | null = content.slice(0,-3).lastIndexOf("bytes");
                                ...
                                if (obj.major === 1) {
                                    ...
                                    let receivedIndex: number | null = obj.content.toLowerCase().split(" ").indexOf("received");
                                    let points: number | null;


                                    if (receivedIndex !== null && receivedIndex !== -1 ) {
                                        try {
                                            points = parseInt(obj.content.toLowerCase().split(" ")[receivedIndex + 1])
                                        } catch {
                                            points = null;
                                        }
                                        obj.points = points;
                                ...
                                if (obj.major === 0) {
                                    ...
                                    if (obj.title.includes("made a new score")) {
                                        ...
                                        let byteIndex: number | null = obj.content.toLowerCase().replace(/[^\d\w\s]/g, '').split(" ").indexOf("bytes");
                                        let newBytes: number | null;
    
                                        if (byteIndex !== null && byteIndex !== -1) {
                                            newBytes = parseInt(obj.content.toLowerCase().replace(/[^\d\w\s]/g, '').split(" ")[byteIndex - 1]);
                                            obj.bytes = newBytes;
                                        }
                            ...
                            try {
                                obj.activity_date = (new Date(obj.activity_date)).toISOString().slice(0, 19).replace('T', ' ');
                            } catch {
                                obj.activity_date = 0;
                            }
                    ...
                    .then(async (myArray) => {
                        ...
                        for (let i = 0; i < 401; i++) {
                            ...
                            s += myArray.map(
                                (obj: any) => {
                                    ...
                                    (
                                        ...
                                        ${obj.oldUserId ?? "NULL"},
                                        ${obj.lang === null ? "NULL" : `'${obj.lang?.toLowerCase()}'`},
                                        ${obj.problem_id ?? "NULL"},
                                        ${obj.points ?? "NULL"},
                                        ${obj.owner_id ?? "NULL"},
                            ...
                            ).slice(100 * i, 100 * -~i).join(',')
        
                            // console.log(s);


                            await conn.execute(s, (err2, rep2) => {
                                console.error(err2);
                                console.error(rep2);
                            });
                        }
