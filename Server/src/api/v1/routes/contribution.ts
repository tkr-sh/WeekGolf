...
export const voteContribution = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const token = req.token;
  let { up } = req.body as RequestType;
  ...
  const userVoted = await sqlExec(
    "SELECT up FROM UpvoteContribution WHERE contribution_id = ? AND owner_id = ?",
    [id, userId],
  );
...
export const voteCommentContribution = async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Did the user already upvoted ?
  const userVoted = await sqlExec(
    "SELECT * FROM UpvoteCommentContribution WHERE comment_id = ? AND owner_id = ?",
    [id, userId],
  );
...
export const createContribution = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (fix.length > 512) return res.status(400).send("The maximum number of random case is of 512.");
  ...
  await sqlExec(
    `INSERT INTO Contribution (
        ...
        VALUES (?, ?, ?, ?, ?, ?)`,
    [title, description, example, moreInfo, ownerId, fix.length],
  ).catch(() => {
    return res
      .status(500)
      .send("An error occured while trying to insert the contribution in the database");
  });
  ...
  if (
    getProblemInsertedResult === undefined ||
    getProblemInsertedResult.length === 0 ||
    getProblemInsertedResult[0]?.id === undefined
  ) {
    return res
      .status(500)
      .send("An error occured whiel trying to get the ID of the contribution you just created");
  }

  // Insert every test case in the database
  totalJSON.forEach(async (e, i) =>
    conn.execute(
...
export const changeState = async (req: AuthenticatedRequest, res: Response) => {
  ...
  conn.execute("SELECT id FROM Users WHERE token = ?", [token], async (err, rep) => {
    ...
    // Else, we still need to verify that the request is valid!
      ...
      // If it's not accepted: Just say it has been refused
        ...
        conn.execute("UPDATE Contribution SET state = -1 WHERE id = ?", [id], (err2, _) => {
          if (err2) {
      ...
      // If the contribution is accepted
        ...
        const [{ lang: lotw }] = await sqlExec(
          "SELECT lang FROM CurrentLang ORDER BY RAND() LIMIT 1",
        );
        ...
        const [{ date_end: date, id: lastId }] = await sqlExec(
          "SELECT date_end, id FROM Problems ORDER BY date_end DESC LIMIT 1",
        );
...
export const editContribution = async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (owner_id !== request_id) {
    return res
      .status(401)
      .send(
        "You're not the author of that problem. Therefore, you don't have the right to modify it.",
      );
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    if (
      ...
      req.body[keys[i]] !== undefined &&
      typeof req.body[keys[i]] === "string"
    ) {
  ...
  const [{ fix_case }] = await sqlExec("SELECT fix_case FROM Contribution WHERE id = ? LIMIT 1", [
    id,
  ]);
  ...
  if (fix !== undefined && fix !== null && verifyValidityTestCaseJSON(fix)) {
    ...
    return res.status(415).send(
      `JSON for fix test case is invalid. The format should be  [{\"input\": \"...\", \"output\": \"...\"}, ...].
Check for mispelling error`,
  ...
  if (random !== undefined && random !== null && verifyValidityTestCaseJSON(random)) {
    ...
    return res.status(415).send(
      `JSON for random test case is invalid. The format should be  [{\"input\": \"...\", \"output\": \"...\"}, ...].
Check for mispelling error`,
    );
  }
