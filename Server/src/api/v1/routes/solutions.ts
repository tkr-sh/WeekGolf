 ...
 */
export const getPreviousSolution = async (
    ...
    (err, rep: any[]) => {
      if (err || rep === undefined) {
        res
          .status(500)
          .send(
            "An error occured while trying to get the best solution for that problem and lang.",
          );
      } else if (rep.length === 0) {
...
const calculateNumberOfPoint = (
  newBytesNb: number,
  beforeBytesNb: number,
  total: number,
): number => {
...
const getSizeSolution = (code: string, lang: string): number => {
  ...
  if (!["vyxal", "apl", "jelly", "bqn"].includes(lang)) {
    ...
    switch (lang) {
      ...
      case "apl":
        SBCS =
          "⌶%'⍺⍵_abcdefghijklmnopqrstuvwxyz¯.⍬0123456789⊢$∆ABCDEFGHIJKLMNOPQRSTUVWXYZ?⍙ÁÂÃÇÈÊËÌÍÎÏÐÒÓÔÕÙÚÛÝþãìðòõ{}⊣⌷¨ÀÄÅÆ⍨ÉÑÖØÜßàáâäåæçèéêëíîïñ[/⌿\\⍀<≤=≥>≠∨∧-+÷×?∊⍴~↑↓⍳○*⌈⌊∇∘(⊂⊃∩∪⊥⊤|;,⍱⍲⍒⍋⍉⌽⊖⍟⌹!⍕⍎⍫⍪≡≢óôöø\"#&┘┐┌└┼─├┤┴┬│@ùúû^ü`:⍷⋄←→⍝)]⎕⍞⍣\n ⊆⍠⍤⌸⌺⍸⍥⍢√⊇…⌾⍮⍭⍧⍛";
        break;
      case "jelly":
        SBCS =
...
export const submitSolution = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  let { code }: submitSolutionType = req.body;
  const { lang, id: problemId }: submitSolutionType = req.body;
  ...
  conn.execute(
    ...
    async (err, rep) => {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (typeof data.err[i] === "string" && data.err[i].length > 0) errors.push(data.err[i]);
      ...
      if (success) {
        ...
        if (
          personnalBestSizeResult.length === 0 ||
          ...
          if (bestSize > sizeCurrentSolution) {
            ...
            const previousUserResult =
              dataSize.length > 0
                ? await sqlExec("SELECT username as name FROM Users WHERE id = ? LIMIT 1", [
                    dataSize[0].owner_id,
                  ])
                : null;
            ...
            const webhookData = {
              title: `${name} made a${upgrade ? "n upgrade" : " new best score"} with ${
                formatLang[lang.toLowerCase()]
              }`,
              upgrade: +upgrade,
              params: {
                ...
                before_name:
                  previousUserResult === null || !previousUserResult.length
                    ? null
                    : previousUserResult[0].name,
