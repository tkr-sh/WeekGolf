    ...
    await conn.execute(
        ...
        async (err, rep) => {
            ...
            const [{timeRemaining}] = await sqlExec(
                `SELECT phase${rep[0].current_phase + 1 === 5 ? "end" : rep[0].current_phase + 1} as timeRemaining
                FROM Phases
                WHERE NOW() > phase1
                ORDER BY id DESC
                LIMIT 1`
                ...
                case 1:
                    await conn.execute(`
                        ...
                        (err2, rep2) => {
                            ...
                            res.json({languages: rep2, phase: rep[0].current_phase, timeRemaining});
                        }
                    );
                    break;
...
export const submitLanguage = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    // vvvvvvvvvvvvvvvvvvv
    const bad_words = ["shit", "bitch", "asshole", "bastard", "cunt", "dick", "pussy", "whore", "slut", "faggot", "nigger", "retard", "spastic", "mongoloid", "cripple", "moron", "idiot", "imbecile", "stupid", "dumb", "ignorant", "fatso", "ugly", "weirdo", "psycho", "nutjob", "loon", "crazy", "enculé", "enculée", "connard", "connasse", "salope", "putain", "pute", "merde", "enculer", "niquer", "baiser", "trou du cul", "bite", "foutre", "suce", "sucer", "salaud", "chiennasse", "pétasse", "enculage", "enculée", "nique", "baise", "salop", "branleur", "branleuse", "trouduc", "pédé", "pédale", "pd", "tapette", "folle", "folle furieuse", "folle dingue", "folle à lier", "malade mentale", "malade", "tarée", "taré", "tarée mentale", "taré mental", "handicapé", "handicapée", "mongol", "mongole", "débile", "débile mentale", "débile mental", "con", "conne", "imbécile", "abruti", "abruti(e)", "couillon", "couillonne", "crotte", "enculé de ta mère", "pute de ta mère", "nique ta mère", "fils de pute", "fils de chienne", "connard de ta mère", "merdeux", "merdeuse", "merdeuse", "va te faire enculer", "va te faire foutre", "va niquer ta mère", "t'es qu'un connard", "t'es qu'une connasse", "t'es qu'une salope", "t'es qu'un enfoiré", "je m'en bats les couilles", "je m'en bats les steaks", "je m'en fous", "j'en ai rien à foutre", "j'en ai rien à cirer", "j'en ai rien à taper", "je m'en bats l'oeil", "je m'en bats les reins", "je m'en bats les c*******", "aller se faire voir", "aller au diable", "aller se faire foutre", "aller se faire enculer", "aller se faire niquer", "vas te faire voir", "vas au diable", "vas te faire foutre", "vas te faire enculer", "vas te faire niquer", "nique ta mère", "nique ta grand-mère", "nique ta race", "suce ma bite", "suce ma queue", "suce moi", "baise ta mère", "baise ta grand-mère", "baise ta race", "ferme ta gueule", "ferme ta bouche", "ta gueule", "nique toi", "va niquer ta mère", "va niquer ta grand-mère", "va niquer ta race", "tête de gland", "cul terreux", "cul terre à terre", "trouduc", "casse-toi", "dégage", "fous le camp", "va-t-en", "connard", "connasse", "salope", "putain", "pute", "merde", "enculer", "niquer", "baiser", "trou du cul", "bite", "foutre", "suce", "sucer", "salaud", "chiennasse", "pétasse", "enculage"]
    ...
    if (bad_words.includes(lang) || bad_words.some(word => lang.includes(word))) {
        ...
        await conn.execute(
            "UPDATE Users SET vote_right = 0 WHERE token = ?",
            [req.token]
        );

        // Notify it to him
        res.status(403).send("We detected that you tried to put innapropriate words and not a language. You don't have the right to add languages anymore. If you think that this is an error, contact an administrator.")
    }
    ...
    await conn.execute(`
        ...
        async (err2, rep2) => {
            ...
            if (rep2[0].current_phase !== 1) {
                res.status(503).send(`You can not suggest new languages during phase ${rep2[0].current_phase}`);
            }
            ...
            await conn.execute(
                ...
                async (err, rep) => {
                    if (err || rep === undefined || rep === null) {
                        res.status(500).send("An internal error occured while trying to get languages with the name of " + lang);
                        console.log(err)
                        return;
                    }
                            ...
                            await conn.execute(
                                ...
                                async (err4, rep4) => {
                                    ...
                                    if ((err4 !== undefined && err4 !== null)  || rep4 === undefined) {
                                        res.status(500).send("An internal error occured while trying to get the number of suggested lang.");
                                        console.error(err4);
                                        return;
                                    }
                                    ...
                                    if (rep4.length === 0 || rep4[0].suggestions < 3) {
                                        // Insert it in the DB
                                        await conn.execute("INSERT INTO SuggestionLang (lang, owner_id, phase_id) VALUES (?, ?, ?)", [lang, rep3[0].id, rep2[0].id]);
...
export const voteLanguage = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    ORDER BY id DESC
    LIMIT 1`)
    .catch(err => {res.status(500).send("An internal error occured."); return;});
    ...
    if (rows1 === null || rows1.length === 0 || !rows1.map(l => l.lang.toLowerCase()).includes(lang.toLowerCase())) {
    ...
    if (voted) {
        ...
        await sqlExec(
            `DELETE FROM UpvoteLang WHERE lang = ? AND owner_id = ? AND phase_id = ? AND final = ${+(current_phase === 3)};`,
            [lang, owner_id, id]
        );
 ...
 */
export const getPersonnalUpvotes =  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
