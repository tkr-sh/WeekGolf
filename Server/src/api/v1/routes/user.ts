...
// E-mail API
import("@elasticemail/elasticemail-client").then((e) => {
  console.log(e);
  elasticEmailVar = e;
});
    ...
    async (err, rep: any[]) => {
      ...
      conn.execute(
        ...
        async (err2, rep2) => {
          ...
          conn.execute("SELECT COUNT(*) FROM CurrentLang", (err3, rep3) => {
            // Check if the database query returned an undefined or non-array response
          ...
          conn.execute("SELECT COUNT(*) FROM Users;", (err3, rep3) => {
            // Check if the database query returned an undefined or non-array response
            if (rep3 === undefined || rep3 === null || !(rep3 instanceof Array)) {
              // If the response is undefined or non-array, send a 500 (Internal Server Error) response with an error message
              res.status(500).send("An error occurred with the number of users");
          ...
          conn.execute(
            `SELECT COUNT(*) FROM Users WHERE golf_score + upgrade_score + coop_score > ${score}`,
            (err3, rep3) => {
              // Check if the database query returned an undefined or non-array response
              if (rep3 === undefined || rep3 === null || !(rep3 instanceof Array)) {
                // If the response is undefined or non-array, send a 500 (Internal Server Error) response with an error message
          ...
          Object.keys(languageScore).map(async (k) => {
            conn.execute(
              ...
              async (errLang, rank) => {
                conn.execute(
                  ...
                  (errLangTot, tot) => {
                    data[k.split("_")[0] + "_rank"] =
                      rank[0]["COUNT(*)"] + 1 + "/" + tot[0]["COUNT(*)"];
    ...
    conn.execute(
      ...
      (err, rep) => {
        ...
        if (rep === undefined || rep === null || !(rep instanceof Array)) {
          res
            .status(500)
            .send("An error occured while trying to get the preview of the best solutions");
          return;
        }
  ...
  // Else, if we need to have more information and filter
    ...
    if (filterType !== undefined) {
      ...
      if (filterType?.size !== undefined) {
        ...
        arrayParamaters = [...arrayParamaters, parseInt(filteredScore)];
      }
      ...
      if (filterType?.rank !== undefined) {
        ...
        arrayParamaters = [...arrayParamaters, parseInt(filteredRank)];
      }
      ...
      if (filterType?.lang !== undefined) {
        ...
        arrayParamaters = [...arrayParamaters, filterType.lang];
      }
      ...
      if (filterType?.problem !== undefined) {
        ...
        // Add the lang to the filter parameters
        arrayParamaters = [...arrayParamaters, filterType.problem];
      }
    }
...
export const getCommentsOfUser = async (
  ...
  conn.execute("SELECT * FROM Comments WHERE owner_id = ?", [id], (err, rep: any[]) => {
    try {
      if (err || rep === undefined) {
        res.status(500).send("An error occured.");
      } else if (rep.length === 0) {
...
export const getActivity = async (req: Request, res: Response): Promise<void> => {
  ...
  if (filterType !== undefined) {
    ...
    if (filterType?.problem !== undefined) {
      ...
      querySQL +=
        " AND problem_id = (SELECT id FROM Problems WHERE title LIKE CONCAT('%', ?, '%'))";
    ...
    if (filterType?.player !== undefined) {
      ...
      // Add the lang to the filter parameters
      arrayParamaters = [...arrayParamaters, filterType.player];
    }
  }
...
export const createAccount = async ({
  ...
  do {
    ...
    alreadyExists = await sqlExec("SELECT username FROM Users WHERE username = ? LIMIT 1", [
      newName,
    ]);
    i++;
  ...
  conn.execute(
    `
		...
		VALUES            (?    , ?       , ?    , ?  , ?  , ?  , ?      , ?        , ?       , ?         , ?      )`,
    [token, name, email, pfp, bio, pwd, country, github_id, stack_id, discord_id, discord].map(
    ...
    conn.execute(
      ...
      (err) => {
        ...
        const emailObject = elasticEmailVar.EmailMessageData.constructFromObject({
          Recipients: [new elasticEmailVar.EmailRecipient(email)],
          Content: {
            Body: [
...
export const login = async (req: Request, res: Response): Promise<void> => {
  ...
  // The result
  const result = await sqlExec("SELECT token, pwd FROM Users WHERE username = ? OR email = ?", [
    name,
    name,
  ]);
  ...
  if (validPwd) {
    ...
    bcrypt.hash(pwd, saltRounds, async (err, hash) => {
      ...
      conn.execute("UPDATE Users SET pwd = ? WHERE token = ?", [hash, token], (err, rep) => {
        if (err) {
    ...
    // else (new password) decode it with bcrypt.
    ...
    async (err, rep: any[]) => {
      if (err || rep === undefined || rep.length === 0) {
        ...
        await createAccount({
          ...
          .then((token) => res.json({ valid: true, token }))
          .catch(() =>
            res.status(500).send("An error occurred while trying to confirm your account."),
          );
      }
    },
  );
...
export const createAccountRequest = async (req: Request, res: Response): Promise<void> => {
  ...
  let { name, email } = req.body as { [key: string]: string };
  const { pwd } = req.body as { [key: string]: string };
  ...
  if (
    email.length < 5 ||
    email.length > 256 ||
    !/^[\w\d._%+-]+@[\w\d.-]+\.[\w.]{2,}$/.test(email)
  ) {
    res
      .status(400)
      .send("The length of the email should be in [5; 256] chars and it should be a valid e-mail.");
    return;
  }
...
export const updateInfo = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  switch (type) {
    ...
    case "email":
      ...
      [{ exist }] = await sqlExec("SELECT EXISTS(SELECT * FROM Users WHERE email = ?) AS exist", [
        data.toLowerCase(),
      ]);
