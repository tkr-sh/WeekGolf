...
export const githubCallback = async (req: AuthCallbackRequest, res: Response) => {
    ...
    const exists = await sqlExec(`SELECT token FROM Users WHERE github_id = ? AND github_id IS NOT NULL LIMIT 1`, [id]);
...
export const discordCallback = async (req: AuthCallbackRequest, res: Response) => {
    ...
    const exists = await sqlExec(`SELECT token FROM Users WHERE discord_id = ? AND discord_id IS NOT NULL LIMIT 1`, [id]);
...
export const stackCallback = async (req: AuthCallbackRequest, res: Response) => {
    ...
    const exists = await sqlExec(`SELECT token FROM Users WHERE stack_id = ? AND stack_id IS NOT NULL LIMIT 1`, [id]);
