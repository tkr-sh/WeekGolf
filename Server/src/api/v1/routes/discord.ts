 ...
 */
export const initLinkDiscordAccount = async (req: SessionRequest, res: Response): Promise<void | Response> => {
	...
	const discordUrl: string = `https://discord.com/api/oauth2/authorize?client_id=${process.env.DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=identify%20email`;
 ...
 */
export const linkDiscordAccount = async (req: SessionRequest, res: Response): Promise<void | Response> => {
 ...
 */
export const updateDiscordDisplay = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
	...
	try {
		...
		await conn.execute(
            ...
            (err, rep) => {
                if (err) {
                    res.status(500).send("An error occured while trying to update the display of your discord account.");
                } else {
                    res.send("OK.")
                }
            }
